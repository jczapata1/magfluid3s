# Configurations
from libs.base.constants import π
from numba import njit
import numpy as np

#------------------------------------------------------------------------------

# Core Radii
@njit
def configuration_Rm(N, RM, σRM):
    '''
    Generate a configuration of core radii with a log-normal distribution.

    Input:
    -                            N (int): Number of Particles
    -                         RM (float): Core Radius
    -                        σRM (float): Standard Deviation of log(RM)

    Output:
    - Rm ((float, ), numpy.ndarray[N, ]): Core Radii

    Used by:
    - libs.base.initialize.initial_Microstates
    - libs.base.initialize.initial_MvsH
    - libs.base.initialize.initial_MvsT

    Last Updated: 
    - 16/08/2026
    '''

    # Rm-Array
    Rm = np.ones(N) 

    # Log-Normal Distribution
    for i in range(N):
        Rm[i] = np.random.lognormal(np.log(RM), σRM) 
        
    return Rm

#------------------------------------------------------------------------------

# Particle Radii
@njit
def configuration_Rp(N, δ, σδ, Rm):
    '''
    Generate a configuration of particle radii with a log-normal distribution.

    Input:
                                 N (int): Number of Particles
    -                          δ (float): Shell Thickness
    -                         σδ (float): Standard Deviation of log(δ)
    - Rm ((float, ), numpy.ndarray[N, ]): Core Radii

    Output:
    - Rp ((float, ), numpy.ndarray[N, ]): Particle Radii

    Used by:
    - libs.base.initialize.initial_Microstates
    - libs.base.initialize.initial_MvsH
    - libs.base.initialize.initial_MvsT

    Last Updated: 
    - 16/08/2026
    '''

    # Rp-Array
    Rp = np.zeros(N) 

    # Log-Normal Distribution
    for i in range(N):
        Rp[i] = Rm[i] + np.random.lognormal(np.log(δ), σδ) 
        
    return Rp

#------------------------------------------------------------------------------

# Core/Particle Volumes
@njit
def configuration_Ω(N, R):
    '''
    Generate a configuration of core/particle volumes.

    Input:
    -                           N (int): Number of Particles
    - R ((float, ), numpy.ndarray[N, ]): Core/Particle Radii

    Output:
    - Ω ((float, ), numpy.ndarray[N, ]): Core/Particle Volumes

    Used by:
    - libs.base.initialize.initial_Microstates
    - libs.base.initialize.initial_MvsH
    - libs.base.initialize.initial_MvsT

    Last Updated: 
    - 16/08/2026
    '''    

    # Ω-Array
    Ω = np.zeros(N) 

    # Spherical Volume
    for i in range(N):
        Ω[i] = (4.0*π/3.0) * R[i]**3
        
    return Ω

#------------------------------------------------------------------------------

# Magnetic Moments (Magnitude)
@njit 
def configuration_μ(N, MS, Ωn):
    '''
    Generate a configuration of magnetic moments (magnitude).

    Input:
    -                            N (int): Number of Particles
    -                         MS (float): Core Saturation Magnetization
    - Ωn ((float, ), numpy.ndarray[N, ]): Core Volumes

    Output:
    -  μ ((float, ), numpy.ndarray[N, ]): Magnetic Moments (Magnitude)

    Used by:
    - libs.base.initialize.initial_Microstates
    - libs.base.initialize.initial_MvsH
    - libs.base.initialize.initial_MvsT 

    Last Updated: 
    - 16/08/2026
    '''     

    # μ-Array
    μ = np.zeros(N) 

    # Magnetic Moment (Magnitude)
    for i in range(N):
        μ[i] = MS * Ωn[i] 
        
    return μ

#------------------------------------------------------------------------------

# Random/Oriented Unitary Vectors
@njit 
def configuration_e(N, θ):   
    ''' 
    Generate a configuration of random/oriented unitary vectors.

    Input: 
    -                            N (int): Number of Particles
    -                          θ (float): Polar Angle

    Output:
    - e ((float, ), numpy.ndarray[N, 3]): Random/Oriented Unitary Vectors

    Used by:
    - libs.base.initialize.initial_Microstates
    - libs.base.initialize.initial_MvsH
    - libs.base.initialize.initial_MvsT

    Last Updated: 
    - 16/08/2026
    '''     

    # e-Array  
    e = np.zeros((N, 3))  

    # Polar Angle and Azimuthal Angle   
    for i in range(N):
        if (θ >= 0.0 and θ <= 180.0): 
            theta = (θ/180.0) * π # Oriented
        else:
            theta = np.random.uniform(0.0, π) # Random

        phi     = np.random.uniform(0.0, 2.0*π) # Random
        e[i, 0] = np.sin(theta) * np.cos(phi)   # ex
        e[i, 1] = np.sin(theta) * np.sin(phi)   # ey
        e[i, 2] = np.cos(theta)                 # ez 
        
    return e